...
AIOP a été créé pour répondre à un besoin croissant de **simplification du processus de packaging** pour les systèmes embarqués. Les développeurs et les équipes de développement sont confrontés à des défis de plus en plus complexes lorsqu'il s'agit de créer des packages pour des systèmes embarqués, des applications, des logiciels, etc. Ces défis sont liés à la diversité des plateformes, des configurations, des environnements de déploiement et des [registres d'artefacts](/docs/vocabulary#registres-dartefacts). AIOP a été conçu pour relever ces défis et offrir une solution complète et puissante pour simplifier le processus de packaging.


## Les cas d'usages

Imaginons que vous ayez un projet qui a besoin d'être déployé sur différents systèmes. Prenons le cas d'un vehicule autonomes. Vous avez une ligne de produit composée de 4 véhicules différents. Ils ont tous les 4 des dimensions différentes et des capteurs et intégrations différentes. En revanche ils ont une base commune de librairie, application, configuration, etc. Avec AIOP, vous pourrez alors déclarer les ressources communes ainsi que les différentes configurations spécifiques à chaque véhicule dans des fichiers de déclaration. Comme un livre de recette, AIOP récupérera les ressources communes et spécifiques à chaque véhicule et les déploiera dans le système approprié.

Demain si vous souhaitez mettre à jour la librairie, vous n'aurez à le mettre à jour qu'à un seul endroit dans les déclarations.
Si nous continuons dans la logique il est possible d'ajouter la notion d'environnemenents et construire des packages pour des environnemenents de dev, pré-prod, prod, etc... qui auront chacun des outils de test ou des connexions à des serveurs différents.

## Comment démarrer

<Cards num={3}>
  <Card href="/docs/playbook_structure" title="Comment cela fonctionne ?" image arrow>
...
- **Traçabilité:** Chaque package est tracé et permet de savoir exactement quand, comment{/* , qui l'a généré*/} et dans quelles conditions. ([Traçabilité](/docs/declarations/overview#base-dune-déclarationy))

### Flexibilité
- **Configuration:** onçus pour offrir une configuration maximale, les packages générés par AIOP peuvent être adaptés à une multitude de cas d'utilisation. ([Flexibilité](/docs/declarations/selection))
- **Personnalisation:** AIOP permet aux développeurs de personnaliser les packages en fonction de leurs besoins spécifiques, en ajoutant ou en supprimant des ressources, en modifiant les paramètres de configuration, etc. ([Personnalisation](/docs/configurations/user))
- **Extensibilité:** AIOP est conçu pour être extensible, ce qui signifie que les développeurs peuvent ajouter des fonctionnalités supplémentaires à l'outil pour répondre à des besoins spécifiques. ([Extensibilité](/docs/aiop-lib/plugins))
...
- **Intégration:** La force de AIOP est de pouvoir récupérer des artefacts de différents registres d'artefacts ([Artifactory](https://jfrog.com/fr/artifactory/), [Nexus](https://www.sonatype.com/products/sonatype-nexus-repository) pour le moment) et de les intégrer dans le package final. ([Intégration](/docs/configurations/playbook#repositories))
...
- **Discord:** Rejoignez notre [serveur Discord](/slack) pour discuter avec d'autres utilisateurs, poser des questions et partager vos expériences.
...
- **Mail:** Contactez-nous par [mail](mailto:support@aiop.io) pour toute question, feedback ou demande d'assistance.

Si vous souhaitez discuter d'un cas d'usage ensemble, contactez nous par [email](mailto:support@aiop.io) ou [prenez rendez-vous](/schedule-demo).
