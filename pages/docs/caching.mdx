...
Pour s’assurer que la resource du cache est à jour Aiop vérifie si la resource distante a été modifiée via le hachage ([En savoir plus sur le hachage](https://www.cnil.fr/fr/securite-chiffrer-garantir-lintegrite-ou-signer)). Si c'est le cas, Aiop télécharge la nouvelle version et met à jour le cache.

À chaque fois qu’une resource est récupérée du serveur distant, il compare le hachage de la resource distante avec le hachage de la resource en cache pour garantir l’intégrité de la resource. Sinon il la retélécharge ou remonte au erreur après plusieurs tentatives. Le nombre de tentatives est configurable ([En savoir plus sur la configuration](/docs/configurations/user#structure-du-fichier-de-configuration)).
