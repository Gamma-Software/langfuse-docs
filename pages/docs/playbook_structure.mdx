---
description: See the basic structure of a playbook. A project of Aiop.
---
...
Voici un example de structure d’un `playbook`:
...
Pour cet example, le dossier nommé `resources` contient les ressources du playbook. Il est bien sur optionnel et vous pouvez le nommer comme vous le souhaitez. Il contient un dossier `locales` avec deux fichiers avec l’extension `.aml`. Le dossier `locales` contient deux resources locales.
