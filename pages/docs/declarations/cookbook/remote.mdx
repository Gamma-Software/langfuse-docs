...
Vous pouvez récupérer n’importe quel type de resource.
...
Prenons l'example de fichier de configuration du playbook suivante. Concentrons nous sur les deux premiers élements de la liste de `repositories`:
...
<Callout type="info" emoji="ℹ️">
Vous constaterez qu’il n’y a pas de resource en local dans le dossier `resources` car tout est à distance sur le serveur.
</Callout>

```yaml {1, 6} filename="resources/.aml" /<artifactory-release>/ /<artifactory-develop>/
...
Les paramétrages ont été fait plus haut, nous pouvons maintenant récupérer des ressources depuis le serveur distant d’`artifactory`. Voici un example concret pour récupérer un fichier de paramètres depuis le dépôt `artifactory-release`:
...
```yaml {1} filename="resources/.aml" /<nexus-develop>/
...
Sachez que les resources distantes sont stockées dans le cache local pour éviter de les récupérer à chaque génération de package ([En savoir plus sur le caching](/docs/caching)). En revanche elles sont vérifiée à chaque génération de package pour garantir que la resources est à jour par rapport à la dernière version disponible sur le serveur distant ([En savoir plus sur la vérification](/docs/cli/build)).
