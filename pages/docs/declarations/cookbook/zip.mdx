---
description: >-
  Archives can be unzipped in runtime.
---
import { FileTree } from 'nextra/components'
import { Callout } from "nextra-theme-docs";

# Dézipper une archive

Lorsque vous avez une archive à dézipper sur un système cible, vous pouvez utiliser le champ optionnel `unzip` pour déclarer cette action.

## Déclaration

Dans le cas où vous avez une archive à dézipper, vous pouvez utiliser le champ `unzip` pour déclarer cette action. Il faut bien entendu que vous déclareriez un fichier avec les extensions suivante:

- `.zip`
- `.tar`
- `.tar.gz`
- `.tar.bz2`
- `.tar.xz`
- `.rar`
- `.7z`

Et que vous indiqueiez le chemin du dossier destination. En effet, il est obligatoire de dézipper l'archive dans un dossier.

<FileTree>
  <FileTree.Folder name="playbook" defaultOpen>
    <FileTree.Folder name=".git">
        <FileTree.File name="..." />
    </FileTree.Folder>
    <FileTree.Folder name=".aiop">
        <FileTree.File name="inventory_config.yml" />
        <FileTree.File name="inventory.yml" />
        <FileTree.File name="playbook.yml" />
    </FileTree.Folder>
    <FileTree.Folder name="resources" defaultOpen>
        <FileTree.File name=".aml" />
        <FileTree.File name="archive.zip" />
    </FileTree.Folder>
  </FileTree.Folder>
</FileTree>

```yaml {2, 3} filename="resources/.aml" /dossier/
- source: archive.zip
  destination: dossier/
  unzip: True
  compatibility:
    - ...
```

<Callout type="warning" emoji="⚠️">
 Pour indiquer le chemin du dossier destination, il est obligatoire de terminer par un `/`.
</Callout>

Par défaut, le champ `unzip` est déclaré à `False` et ne dézippera donc pas le fichier. Il est donc nécessaire de le déclarer à `True` pour que l'archive soit dézippée.