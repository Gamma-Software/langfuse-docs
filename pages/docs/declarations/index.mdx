...
Le champ `source` est l’emplacement de la ressource à récupérer. Elle peut être de différente forme. Cela peut être une ressource local et relative au fichier de déclaration ou une ressource distante. Dans le cas d’une ressource distante, elle peut être une URL ou un chemin vers un fichier distant d'un dépôt. Aiop permet également de combiner des resources ([En savoir plus](/docs/declarations/cookbook/combine)).
