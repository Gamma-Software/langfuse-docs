...
La règle `SnakeCaseRule` est définie dans le fichier `.aiop/rules/snake_case_rule.py`. Pour cet example, cette règle vérifie que les noms de fichiers sont en snake_case.
...
Le script `ApplyPatch` est défini dans le fichier `.aiop/rules/apply_patch.py`. Pour cet example, ce script applique un patch sur le package généré.
