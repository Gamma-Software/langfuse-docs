...
In case a plugin requires specific configuration, it should be integrated into the `plugin_config` section. Please refer to the plugin documentation or contact support via [valentin.rudloff.perso@gmail.com] for more information.
...
| logs.level | str | Log level | CRITICAL, FATAL, ERROR, WARNING, WARN, INFO, DEBUG, DISABLED | DEBUG |
| logs.output | str | Log output | stdout, file | stdout |
| logs.pathdir | str | Absolute path to log storage directory | Absolute path | `~/.aiop/logs` |
