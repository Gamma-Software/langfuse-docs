---
description: Cette configuration permet de configurer tous les packages que vous souhaitez générez. Il est utilisé uniquement par la commande `aiop build_all` pour générer tous les packages en une fois.
---
import { Callout } from "nextra-theme-docs";

# Configuration des packages

La configuration des packages est optionnel et permet de configurer tous les packages que vous souhaitez générer. Il est utilisé __uniquement__ par la commande `aiop build_all` pour générer tous les packages en une fois [[En savoir plus](/docs/cli/build#commande-build_all)].

## Configuration

Le fichier de configuration des packages est situé dans le répertoire relatif au playbook:
- Unix: `<racine_playbook>/.aiop`
- Windows: `<racine_playbook>\.aiop`
