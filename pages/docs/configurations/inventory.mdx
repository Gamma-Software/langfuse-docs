...
La section `inventory` contient les systèmes cibles et les variables spécifiques à chaque système. Chaque système cible est défini par un objet avec deux champs: `system` et `compatibility`; similaire à la configuration de l’inventaire. En effet, vous êtes obligé de respecter l’ordre des champs et les valeurs autorisée défini dans le fichier de configuration de l’inventaire.

### Modèle de configuration

Reprenons l’example de la société Tesla utilisé dans le [chapitre précédent](docs/configuration/playbook). Nous pourrions définir le fichier d’inventaire comme suit.
...
Voici un exemple de fichier d’inventaire pour la société Tesla. Nous pouvons voir que l’inventaire propose de générer trois systèmes cibles.
...
Vous pouvez ajouter autant de systèmes cibles que vous le souhaitez dans le fichier d’inventaire. Vous pouvez également ajouter autant de variables spécifiques à chaque système que vous le souhaitez. Tout cela donne des possibilités infinies pour la génération de packages.
...
Reprenons l’example de la société Tesla. Nous pourrions ainsi demander de créer des packages pour :
...
Dans le premier cas, l’utilisateur demande de générer un package pour un Model X avec les fonctionnalités `four-wheel-drive` et `butterfly-doors` fonctionnant dans environnement `dev`. La règle des fonctionnalités étant `least_diff`, l’utilisateur pourra renseigner une ou plusieurs des fonctionnalités autorisées. La règle de l’environnement étant `one_of`, l’utilisateur pourra renseigner une seule des valeurs autorisées. De cette manière Aiop pourra générer un package pour un Model X avec les fonctionnalités `four-wheel-drive` et `butterfly-doors` fonctionnant dans environnement `dev`.