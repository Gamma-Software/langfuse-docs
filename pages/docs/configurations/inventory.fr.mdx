...
L’inventaire est une configuration qui répertorie les systèmes cible et ces caractéristiques. Il est utilisé pour définir les systèmes pour lesquels le playbook à le doit de générer un package. Il est également utilisé pour définir les variables spécifiques à chaque système. Veuillez définir l'inventaire et sa configuration dans le fichier `.aiop` à la racine du playbook.

- `inventory_config`: Le dictionnaire de configuration de l’inventaire.
- `inventory.`: Le dictionnaire de l'inventaire.

## Structure de la configuration de l'inventaire

La configuration de l’inventaire est au format YAML. Il est possible de le modifier avec un éditeur de texte ou un éditeur de code. Il sera ensuite lu et vérifié par AIOP.
...
Nous pouvons voir que le fichier de configuration de l’inventaire contient deux sections dans le champ `inventory_config`:
...
L'inventaire est au format YAML. Il est possible de le modifier avec un éditeur de texte ou un éditeur de code. Il sera ensuite lu et vérifié par AIOP. C'est ce fichier qui contient les systèmes cible et les variables spécifiques à chaque système.
