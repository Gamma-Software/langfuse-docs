...
[En savoir plus](/docs/configurations).
...
<Callout type="warning" emoji="ℹ⚠️">
...
La phase de `build` est responsable de la construction du package. Elle assure l'assemblage des resources dans le package. [En savoir plus](/docs/build).

### Post-build

La phase de `post-build` est utilisée pour réaliser de tâches qui n’ont pas de sens à être inclue dans le phase de `build`. Par example, vous pourriez être amené à vouloir générer un fond d’écran pour un client qui affiche en texte des métadonnées relatives au système cible. Toutes manipulation du packages temporaire après `build` doit être fait dans cette phase. [En savoir plus](/docs/post-build).
