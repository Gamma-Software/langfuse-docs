...
La commande de `build` permet de construire un package basé sur la cible système de l'utilisateur et ses propriétés. Il y a aussi des options que nous allons détailler.

```bash
aiop build {system_target} {params}
```
> `system_target` : La cible système pour laquelle générer le package
...
```ansi
usage: aiop build [-h] [--destination_path DESTINATION_PATH] [--dryrun] [--force] [--no_caching] [--no_verify]
...
| `no_caching` | Ne pas utiliser le cache pour récupérer les resources distantes déjà téléchargées | `False`: Les resources distantes sont copiés depuis le cache, si elles existent |
| `no_verify` | Ne pas vérifier l'intégrité de la resource distante| `False`: Vérifie l'intégrité de l'artifact |
